...
import PlayTypeSelector from "./selector/PlayTypeSelector";
...
export default function TeamCreateModal({
  ...
  const sendCreatePacket = () => {
    if (validate()) {
      ...
      axios
        .post(API_SERVER + "/post", JSON.stringify(packet), {
        ...
        <ModalBody pt={5} w={"100%"}>
          <VStack alignItems={"flex-start"} spacing={8}>
            <FormControl isRequired>
              <FormLabel color={"#AEAEB0"} fontSize={"14px"}>
                디스코드 아이디
              </FormLabel>
              <HStack spacing={4}>
                {profile && (
            ...
            <HStack spacing={5}>
              ...
              <FormControl>
                ...
                <PlayTypeSelector
                  setter={(e: any) => setType(e.target.value)}
                />
              </FormControl>
              <FormControl>
                <FormLabel color={"#AEAEB0"} fontSize={"14px"}>
                ...
                <RegionSelector
                  setter={(e: any) => setServer(e.target.value)}
                />
              </FormControl>
            </HStack>
            <FormControl>
