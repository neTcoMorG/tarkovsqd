import { Box, Container, Image, HStack, Text } from '@chakra-ui/react';
					...
					<HStack spacing={'130px'}>
						...
						<HStack justifyContent={'space-between'} h={'100%'}>
							<HStack spacing={10}>
								<Text
									onClick={() => navigate('/')} 
									fontSize={'15px'} color={ '#9A8866' } cursor={'pointer'}>팀원찾기</Text>
								<Text 
									onClick={() => navigate('/market')}
									fontSize={'15px'} color={ '#9A8866' } cursor={'pointer'}>암시장</Text>
								<Text 
								onClick={() => navigate('/market')}
								fontSize={'15px'} color={ '#9A8866' } letterSpacing={'-1px'} cursor={'pointer'}>문의사항</Text>
							</HStack>
						</HStack>
					</HStack>
					<HStack>
